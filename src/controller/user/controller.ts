...
export const loginUser: RequestHandler = async (req, res, next) => {
  try {
    ...
    if (!username || !password) throw new BadRequestError("아이디와 비밀번호를 모두 입력해주세요");
    ...
    // 비밀번호 검증 단계 필요? 해싱한다면 준스처럼 구현하면 될 것 같고
    // 필요없다면 user의 password와 비교 (?)
    if (password !== user[0].password) throw new UnauthorizedError('비밀번호가 옳지 않습니다.');
